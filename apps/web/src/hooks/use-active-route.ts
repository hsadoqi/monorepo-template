...
import { groups, items, type NavItem, type SidebarGroup } from "@/lib/data/navigation";
...
export const resolvePageTitle = (pathname: string, navItems: NavItem[] = items) => {
...
export const resolveActiveGroup = (
  ...
  navGroups.find((group) =>
    group.items.some((item) => item.href && isActiveRoute(item.href, pathname)),
  )?.label;
